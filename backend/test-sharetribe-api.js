...
db.get(
  ...
  async (err, row) => {
    ...
    const authUrl = 'https://api.sharetribe.com/v1/auth/token';
    ...
    try {
      ...
      if (tokenResponse.data && tokenResponse.data.access_token) {
        ...
        console.log('Token expires in:', tokenResponse.data.expires_in || 'unknown', 'seconds');
        console.log('\n');
      } else {
        console.log('❌ Failed to get OAuth2 token');
        console.log('Response:', JSON.stringify(tokenResponse.data, null, 2));
        db.close();
        return;
      }
    } catch (error) {
      console.log('❌ Error getting OAuth2 token:', error.message);
      if (error.response) {
    ...
    console.log('Step 2: Testing ShareTribe Marketplace API with OAuth2 token...');
    console.log('URL:', apiUrl);
    console.log('Method: GET');
    console.log('Auth: Bearer token (OAuth2)');
    console.log('\n');
