...
<GetSellerListRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <Version>1423</Version>
    ...
    try {
      ...
      // Helper to normalize HTML to plain text
      const normalizeHtmlToText = (html) => {
        ...
        // Replace <p> tags with double newlines (paragraph break)
      ...
      const basicProducts = itemsArray
        ...
        .map(item => {
          ...
          // Extract Description and normalize HTML to plain text
          let description = extractTextValue(item.Description) || extractTextValue(item.description) || '';
          
          // Normalize HTML to plain text
          if (description) {
            description = normalizeHtmlToText(description);
          }
  ...
  async getItemSpecifics(itemId) {
    ...
    // Use header authentication (X-EBAY-API-IAF-TOKEN) instead of RequesterCredentials
    const xmlRequest = `<?xml version="1.0" encoding="utf-8"?>
<GetItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  <Version>1423</Version>
    ...
    console.log(`📋 Fetching Item Specifics for ItemID: ${itemId}`);
    ...
    try {
      ...
      const parser = new xml2js.Parser({ 
        explicitArray: false, 
        mergeAttrs: true,
        explicitText: true, // Preserve text content (stores in #text or _)
        trim: true,
        explicitRoot: false,
        ignoreAttrs: false,
        attrkey: '_attr',
      ...
      // Extract GetItemResponse (handle namespaces, same as GetSellerList)
      let getItemResponse = null;
      
      // Check if result itself is the GetItemResponse (has Item, Ack, etc.)
      if (result && (result.Item || result.Ack || result.Timestamp)) {
        getItemResponse = result;
      } else if (result && result.GetItemResponse) {
        getItemResponse = result.GetItemResponse;
      } else if (result && result['soapenv:Envelope']?.['soapenv:Body']?.GetItemResponse) {
        getItemResponse = result['soapenv:Envelope']['soapenv:Body'].GetItemResponse;
      }
      
      if (!getItemResponse) {
        console.error(`❌ Unexpected GetItem response structure for ${itemId}`);
        console.error('   Available keys:', Object.keys(result || {}));
        console.error('   Raw response preview:', String(rawXml).substring(0, 500));
        return {};
      }
      
      // Check for errors
      const ack = getItemResponse.Ack || getItemResponse.ack;
      if (ack === 'Failure' || ack === 'failure' || getItemResponse.Errors) {
        const errors = getItemResponse.Errors;
        if (errors) {
          const errorArray = Array.isArray(errors.Error) ? errors.Error : [errors.Error];
          const errorMessages = errorArray.map(e => {
            const shortMsg = e.ShortMessage || e.shortMessage || '';
            const longMsg = e.LongMessage || e.longMessage || '';
            return `${shortMsg}${longMsg ? ' - ' + longMsg : ''}`;
          }).join('; ');
      ...
      // Extract Item Specifics from Item.ItemSpecifics.NameValueList[]
      const item = getItemResponse.Item;
      
      if (!item) {
        console.log(`⚠️ No Item found in GetItemResponse for ${itemId}`);
        return {};
      }
      
      if (!item.ItemSpecifics) {
        console.log(`⚠️ No ItemSpecifics found for ${itemId}`);
        return {};
      }
      
      const itemSpecifics = {};
      
      // Extract NameValueList array from Item.ItemSpecifics.NameValueList[]
      const itemSpecificsNode = item.ItemSpecifics;
      let nameValueList = itemSpecificsNode.NameValueList;
      
      // Handle different XML structures
      if (!nameValueList) {
        // Try alternative paths
        nameValueList = itemSpecificsNode['NameValueList'] || 
                       itemSpecificsNode['nameValueList'] ||
                       itemSpecificsNode['NameValue'];
      }
      
      if (!nameValueList) {
        console.log(`⚠️ No NameValueList found in ItemSpecifics for ${itemId}`);
        console.log('   ItemSpecifics structure:', JSON.stringify(itemSpecificsNode, null, 2).substring(0, 500));
        return {};
      }
      
      // Helper to extract text value from XML element
      const extractTextValue = (node) => {
        if (!node) return null;
        if (typeof node === 'string') return node.trim();
        if (node['#text'] !== undefined) return String(node['#text']).trim();
        ...
        if (typeof node === 'object' && !Array.isArray(node)) {
          const textKeys = ['#text', '_', '_text', 'text', 'Text'];
      ...
      console.log(`📋 Found ${nameValueArray.length} Item Specifics for ${itemId}`);
      
      // Store Item Specifics as dynamic key-value map (not hardcoded)
      nameValueArray.forEach((nv, index) => {
        // Extract Name (field name like "Brand", "Size", "Color", etc.)
        let name = extractTextValue(nv.Name) || nv.Name;
        if (!name && nv.name) {
          name = extractTextValue(nv.name) || nv.name;
        }
        
        // Extract Value (can be string or array of strings)
        let value = null;
        if (nv.Value !== undefined && nv.Value !== null) {
          if (Array.isArray(nv.Value)) {
            // Multiple values: join with comma
            value = nv.Value.map(v => extractTextValue(v) || v).filter(Boolean).join(', ');
          } else {
            // Single value
            value = extractTextValue(nv.Value) || nv.Value;
          }
        } else if (nv.value !== undefined && nv.value !== null) {
          // Try lowercase 'value' key
          if (Array.isArray(nv.value)) {
            value = nv.value.map(v => extractTextValue(v) || v).filter(Boolean).join(', ');
          } else {
            value = extractTextValue(nv.value) || nv.value;
        ...
        if (name && value !== null && value !== undefined) {
          // Store as dynamic key-value pair
          // Use lowercase key to normalize (e.g., "Brand" -> "brand")
          const fieldKey = String(name).toLowerCase().trim();
          const fieldValue = String(value).trim();
          
          // Store in itemSpecifics object
          itemSpecifics[fieldKey] = fieldValue;
          
          console.log(`   ✅ ${itemId} [${index + 1}/${nameValueArray.length}]: ${name} = ${fieldValue}`);
        } else {
          console.log(`   ⚠️ ${itemId} [${index + 1}/${nameValueArray.length}]: Skipping - name=${name}, value=${value}`);
          console.log(`      Raw node:`, JSON.stringify(nv, null, 2).substring(0, 200));
        }
      });
      
      console.log(`✅ Extracted ${Object.keys(itemSpecifics).length} Item Specifics for ${itemId}:`, Object.keys(itemSpecifics).join(', '));
      
      // Extract Condition (NOT an Item Specific - it's a separate field)
      // Condition is returned reliably by Trading API GetItem
      if (item.ConditionID !== undefined && item.ConditionID !== null) {
        itemSpecifics.conditionid = String(item.ConditionID).trim();
        console.log(`   📦 ${itemId}: ConditionID = ${itemSpecifics.conditionid}`);
      }
      
      if (item.ConditionDisplayName !== undefined && item.ConditionDisplayName !== null) {
        // Use 'condition' as the key for display name (common field name)
        const conditionDisplayName = extractTextValue(item.ConditionDisplayName) || item.ConditionDisplayName;
        itemSpecifics.condition = String(conditionDisplayName).trim();
        console.log(`   📦 ${itemId}: ConditionDisplayName = ${itemSpecifics.condition}`);
      }
      
      // Also store as conditiondisplayname for clarity
      if (item.ConditionDisplayName !== undefined && item.ConditionDisplayName !== null) {
        const conditionDisplayName = extractTextValue(item.ConditionDisplayName) || item.ConditionDisplayName;
        itemSpecifics.conditiondisplayname = String(conditionDisplayName).trim();
      }
